...
def appointments(request):
    ...
    return render(request,"Reception/appointments.html",c)
...
def reception(request):
    ...
    return render(request,"Reception/reception.html",c)
...
@login_required
def deleteapt(request):
    if request.method=='POST':
        aid=request.POST['aid']
        apt=Appointment.objects.filter(aid=aid).first()
        apt.delete()
    ap=Appointment.objects.all()
    p=Patient.objects.all()
    c={'ap': ap ,'p':p}             
...
def createpat(request):
    ...
    return render(request,'Reception/crtpat.html')
...
def getapt(request):
    ...
    if request.method=='POST':
        aid=request.POST['aid']
        apt=Appointment.objects.filter(aid=aid).first()
        return render(request,'Reception/updateapt.html',{'apt':apt,'d':d,'p':p,'m':m,'da':da,'y':y,'h':h})

@login_required
def updateapt(request):
    if request.method=='POST':
        aid=request.POST['aid']
        apt=Appointment.objects.filter(aid=aid).first()
        apt.aid=aid
...
def setapt(request):
    ...
    if request.method=='POST':
        ...
        a.save()
        return redirect('reception')
    return render(request,'Reception/setapt.html',{'d':d,'p':p,'m':m,'da':da,'y':y,'h':h})